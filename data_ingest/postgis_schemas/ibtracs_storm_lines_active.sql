CREATE TABLE IF NOT EXISTS public."ibtracs_active_storm_lines" (
...
CREATE INDEX IF NOT EXISTS ibtracs_active_storm_lines_name_idx
    ON public.ibtracs_active_storm_lines 
    USING btree ("NAME" ASC NULLS LAST)
    WITH (deduplicate_items=True)
    TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS ibtracs_active_storm_lines_season_idx
    ON public.ibtracs_active_storm_lines 
    USING btree ("SEASON" ASC NULLS LAST)
    WITH (deduplicate_items=True)
    TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS ibtracs_active_storm_lines_iso_time_idx
    ON public.ibtracs_active_storm_lines 
    USING btree ("ISO_TIME_START" ASC NULLS LAST, "ISO_TIME_END" ASC NULLS LAST)
    WITH (deduplicate_items=True)
    TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS ibtracs_active_storm_lines_line_idx
    ON public.ibtracs_active_storm_lines 
    USING GIST (geom_storm_line);

CREATE INDEX IF NOT EXISTS ibtracs_active_storm_lines_buffer_idx
    ON public.ibtracs_active_storm_lines 
